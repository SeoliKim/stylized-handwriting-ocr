...
def main():
    ...
    print(f"Project is at : {args.project_dir}")
    print(f"Dataset is at : {args.dataset_dir}")
    ...
    # Filter rows where 'text_column' has length < 16
    df_test = df_test[df_test['text'].str.len() < 16]
    
    # get sample of 10000
    sampled_df = df_test.sample(n=10000, random_state=42).reset_index()
    sampled_df.info()
    
    # Run, test first 10000 cases
    results_df = process_all_rows_batched(sampled_df, model, processor)
